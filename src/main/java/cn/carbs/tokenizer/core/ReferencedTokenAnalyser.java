...
public class ReferencedTokenAnalyser {

    /**
     *
     * @param sealedTokens 一个 code 文件中的所有 token
     * @param standardResourceImportPaths
     *          可能的标准 R 引包路径，比如 cn.carbs.tools.R;
     *          多个不同的library可能有不同的namespace（也可能namespace相同），因此需要把所有的子library的namespace传入，
     *          用于分析资源引用
     * @return
     */
    public static ArrayList<ReferencedToken> analyseReferencedTokensFromSealedTokens(ArrayList<SealedToken> sealedTokens,
                                                                                     ArrayList<String> standardResourceImportPaths) {
        String packagePath = getPackage(sealedTokens);
        ArrayList<String> importsPaths = getImports(sealedTokens);
...
//        }
        ...
        if (standardResourceImportPaths == null) {
            return null;
        }
        ...
        for (String certainRPath : standardResourceImportPaths) {
...
//        System.out.println("==================================================");
        ...
        for (int i = 0; i < sealedTokens.size(); i++) {
            SealedToken currentToken = sealedTokens.get(i);
                ...
                if (prevAvailableTokenType != TokenType.Identifier) {
                    ...
                    if (completeIdentifierToken.size() > 0) {
                        ...
                        if (completeToken.length() >= minLengthOfImportPath) {
                            ...
                            for (String s : standardResourceImportPaths) {
...
//                                System.out.println("[MAYBE] ReferencedToken 5 --> " + referencedToken);
    ...
    public static String getPackage(ArrayList<SealedToken> tokens) {
    ...
    public static ArrayList<String> getImports(ArrayList<SealedToken> tokens) {
